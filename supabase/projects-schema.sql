...
create policy "projects readable by ceo and assignee" on public.projects for select
  using (public.is_ceo() or assigned_engineer_id = auth.uid());
...
create or replace function public.create_project_with_budget(
  ...
  p_engineer uuid, p_budget numeric, p_start date, p_end date,
...
begin
  ...
  insert into public.projects(name, location, client_name, subject, lead, assigned_engineer_id, budget_ceiling, start_date, end_date, description, image_url, created_by, updated_by)
  values (trim(p_name), trim(p_location), nullif(trim(p_client), ''), nullif(trim(p_subject), ''), nullif(trim(p_lead), ''), p_engineer, p_budget, p_start, p_end, nullif(trim(p_description), ''), nullif(trim(p_image_url), ''), p_actor, p_actor)
...
revoke all on function public.create_project_with_budget(uuid,text,text,text,text,uuid,numeric,date,date,text,text,text) from public;
grant execute on function public.create_project_with_budget(uuid,text,text,text,text,uuid,numeric,date,date,text,text,text) to service_role;
