...
const INITIAL_REQUESTS: MaterialRequest[] = [
  ...
  },
  {
    ...
    approvalNotes: "Rejected. Project currently on hold by client decision. Wait for reactivation.",
  },
];
...
export default function MaterialApprovalsPageClient() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setRequests(INITIAL_REQUESTS);
      localStorage.setItem("prodisenyo-material-requests-v2", JSON.stringify(INITIAL_REQUESTS));
    }
  }, []);
  ...
  const handleActionClick = (request: MaterialRequest, action: "approve" | "reject") => {
  ...
  const handleConfirmAction = () => {
    ...
    const targetStatus = (actionType === "approve" ? "approved" : "rejected") as MaterialRequest["status"];
    ...
    startTransition(async () => {
      ...
      const updated = requests.map((r) =>
        r.id === reviewingRequest.id
          ? { ...r, status: targetStatus, approvalNotes: commentText.trim() || undefined }
          : r
      );
      
      setRequests(updated);
      localStorage.setItem("prodisenyo-material-requests-v2", JSON.stringify(updated));

      toast.success(`Request for ${reviewingRequest.materialName} successfully ${statusWord}!`);
  ...
  return (
    <div className="space-y-4 p-0 sm:p-6">
      <DashboardPageHero
        ...
        title="Material Approval Queue"
        description="Verify, approve, or decline material requests submitted by site engineers to regulate inventory and costs."
        actions={
      ...
      <div className="space-y-4">
        {filteredRequests.length > 0 ? (
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-start flex-1">
                  ...
                  <img
                    src={
                      req.materialName.toLowerCase().includes("cement")
                        ...
                        : req.materialName.toLowerCase().includes("bar") || req.materialName.toLowerCase().includes("steel")
                        ? "https://images.unsplash.com/photo-1504917595217-d4dc5ebe6122?auto=format&fit=crop&w=150&q=80"
                        : req.materialName.toLowerCase().includes("glass") || req.materialName.toLowerCase().includes("facade")
                        ? "https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?auto=format&fit=crop&w=150&q=80"
                        : req.materialName.toLowerCase().includes("pipe") || req.materialName.toLowerCase().includes("pvc")
                ...
                <div className="space-y-2 flex-1">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className={cn("border px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider", getPriorityBadgeClass(req.priority))}>
                  ...
                  <div>
                    <h4 className="text-base font-bold text-apple-charcoal">{req.materialName}</h4>
                ...
                {req.approvalNotes && (
                  <div className="bg-emerald-50/20 border border-emerald-100 text-emerald-800 rounded-xl p-2.5 flex gap-2">
                    ...
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-700/80">CEO Review Notes</p>
                      <p className="text-xs italic mt-0.5">{req.approvalNotes}</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center gap-2 self-end md:self-center">
                {req.status === "pending" ? (
                  ...
                  <span className={cn(
                    ...
                    req.status === "approved" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-rose-200 bg-rose-50 text-rose-700"
                  )}>
          ...
          <div className="text-center py-16 bg-white border border-dashed border-slate-200 rounded-2xl">
            ...
            <p className="mt-2 text-sm font-semibold text-slate-500">No requests found</p>
            <p className="text-xs text-slate-400 mt-1">Excellent! All items in this category are completed.</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-white border border-apple-mist rounded-2xl p-6 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <p className="text-xs text-slate-500 mt-1">
              {reviewingRequest.materialName} ({reviewingRequest.quantity} {reviewingRequest.unit}) for {reviewingRequest.projectName}
            </p>
            ...
            <div className="mt-4 space-y-3">
              ...
              <textarea
                ...
                placeholder={actionType === "approve" ? "Provide approval notes, specifications, or vendor hints..." : "Explain reason for rejecting the request..."}
                rows={3}
            ...
            <div className="mt-6 flex justify-end gap-3 pt-3 border-t border-slate-100">
              ...
              >
                {isPendingTransition && <LoaderCircle size={14} className="animate-spin" />}
